<?php
    ...
    if(isset($_SESSION['privilege'])) {
      ...
      exit();
        ...
        if($u=='h')
        $sql = "UPDATE `assignments` SET `soa_assigned` = 1 WHERE `assign_id` =".$assign_id."";
        elseif ($u=='c')
        $sql = "UPDATE `contentwriting` SET `soa_assigned` = 1 WHERE `content_id` =".$assign_id."";
        elseif ($u=='t')
        $sql = "UPDATE `typing` SET `soa_assigned` = 1 WHERE `type_id` =".$assign_id."";
        // var_dump($sql);
