    ...
    <div class="site-section  overlay" id="contact-section">
        ...
        <th>Action</th>
			...
			while ($row = mysqli_fetch_array($result)){
				 echo '<tr>
						 <td>'.$row["user_id"].'</td>
						 <td>'.$row["type_id"].'</td>
						 <td><a href="temp.php?u=vt&f='.$row["file_name"].'">'.$row["file_name"].'</a></td>
                         <td>'.$row["title"].'</td>
                         <td>'.$row["desc"].'</td>
						...
						$check_assigned = mysqli_query($conn, "SELECT soa_assigned,soa_paid,soa_written,soa_completed FROM typing WHERE type_id=".$row['type_id']."");
						// $check_written = ;
						// $check_paid = ;
						// $check_completed = ;
						...
						{
              echo '<td><a class="top" href="includes/assigned.inc.php?id='.$row['type_id'].'&u=t">Not Assigned</a></td>
              <td>Not Paid</td>
              <td>Not Written</td>
              <td>Not Completed</td>
              <td><a href="" class="top"  data-toggle="modal" data-target="#deletemodal'.$row['type_id'].'" >Delete</a></td>';
						}	
						else
						{
							echo '
							...
							if($check['soa_paid']==0)
							{
                echo'<td><a class="top" href="includes/paid.inc.php?id='.$row['type_id'].'&u=t">Not Paid</a>
                <td>Not Written</td>
              <td>Not Completed</td>
              <td><a href="" class="top"  data-toggle="modal" data-target="#deletemodal'.$row['type_id'].'" >Delete</a></td>';
							}
							else
							{
								...
								{
                  echo '<td><a class="top" href="includes/written.inc.php?id='.$row['type_id'].'&u=t">Not Written</a>
                  <td>Not Completed</td>
              <td><a href="" class="top"  data-toggle="modal" data-target="#deletemodal'.$row['type_id'].'" >Delete</a></td>';
								}
								else
								{
									...
									{
                    echo '<td><a class="top" href="includes/complete.inc.php?id='.$row['type_id'].'&u=t">Not Completed</a></td>
                    <td><a href="" data-toggle="modal" data-target="#deletemodal'.$row['type_id'].'" class="top">Delete</a></td>';
									}
									else
									{
										echo '
                    ...
                    <td><a href="" data-toggle="modal" data-target="#deletemodal'.$row['type_id'].'" class="top">Delete</a></td>';
					 ...
					 echo'</tr> <div class="modal fade" id="deletemodal'.$row['type_id'].'" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <a href="includes/delete.inc.php?id='.$row['type_id'].'&u=t"><button type="button" class="btn btn-primary">Yes</button></a>
